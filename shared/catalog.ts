...
export const CATEGORIES: CategoryEntry[] = [
  ...
  { id: "automotive", thumb: "cells",
    ...
    guide: {
      ko: "추천 순서 — EV 배터리 → 구동 모터 → 내연기관. 전기 동력계(저장 → 구동)를 먼저 본 뒤, 같은 일을 연료의 폭발로 해내는 내연기관과 비교해 보세요.",
      en: "Suggested order — EV Battery → Drive Motor → Combustion Engine. See the electric powertrain first (store → drive), then compare with the engine doing the same job through combustion.",
      ja: "おすすめ順 — EV バッテリー → 駆動モーター → 内燃エンジン。電動パワートレイン（蓄える → 駆動する）を先に見て、同じ仕事を燃料の爆発でこなす内燃機関と比べてみてください。",
      zh: "推荐顺序 — EV 电池 → 驱动电机 → 内燃机。先看电动动力系统（储能 → 驱动），再与用燃料爆燃完成同样工作的内燃机对比。",
    } },
...
export const MODELS: ModelEntry[] = [
  ...
  { cat: "automotive", id: "combustion-engine", status: "live", thumb: "cells",
    name: { ko: "내연기관 엔진", en: "Combustion Engine", ja: "内燃エンジン", zh: "内燃机" },
    desc: { ko: "4행정 작동 원리", en: "Four-stroke cycle", ja: "4ストロークの原理", zh: "四冲程原理" },
    overview: {
      ko: "100년 넘게 자동차를 움직여 온 직렬 4기통 가솔린 엔진입니다. 각 피스톤은 흡입 → 압축 → 폭발 → 배기의 4행정을 반복하고, 폭발의 직선 운동을 커넥팅 로드가 크랭크샤프트의 회전으로 바꿉니다. 네 실린더가 1-3-4-2 순서로 번갈아 폭발해 회전이 끊기지 않아요. 모델의 피스톤 4개 높이가 서로 다른 것이 바로 그 위상 차이입니다. 전기 동력계(배터리·모터)와 비교하며 보면, 두 방식이 같은 문제 — 에너지를 바퀴의 회전으로 — 를 어떻게 다르게 푸는지 보입니다.",
      en: "The inline-four gasoline engine has moved cars for over a century. Each piston repeats four strokes — intake → compression → power → exhaust — and the connecting rods turn that straight-line blast into the crankshaft's rotation. The four cylinders fire in 1-3-4-2 order so the spin never stalls; the different piston heights in this model show exactly that phasing. Compared with the electric powertrain (battery & motor), it reveals two different answers to the same problem: turning energy into wheel rotation.",
      ja: "100 年以上にわたり車を動かしてきた直列 4 気筒ガソリンエンジンです。各ピストンは吸気 → 圧縮 → 燃焼 → 排気の 4 ストロークを繰り返し、爆発の直線運動をコネクティングロッドがクランクシャフトの回転に変えます。4 気筒が 1-3-4-2 の順で交代に燃焼するため回転は途切れません。モデルの 4 つのピストンの高さの違いがまさにその位相差です。電動パワートレイン（バッテリー・モーター）と見比べると、同じ問題 — エネルギーを車輪の回転に — への異なる答えが見えてきます。",
      zh: "直列四缸汽油机驱动汽车已逾百年。每个活塞重复进气 → 压缩 → 做功 → 排气四个冲程，连杆把爆燃的直线运动变成曲轴的旋转。四缸按 1-3-4-2 顺序轮流做功，旋转永不间断——模型中四个活塞高度不同，正是这种相位差。与电动动力系统（电池·电机）对照来看，能看出两种方式如何以不同思路解决同一问题：把能量变成车轮的旋转。",
    },
    specs: [
      { label: { ko: "형식", en: "Layout", ja: "形式", zh: "形式" }, value: "inline-4 · DOHC" },
      { label: { ko: "배기량", en: "Displacement", ja: "排気量", zh: "排量" }, value: "≈ 2.0 L" },
      { label: { ko: "사이클", en: "Cycle", ja: "サイクル", zh: "循环" }, value: "4-stroke (Otto)" },
      { label: { ko: "압축비", en: "Compression", ja: "圧縮比", zh: "压缩比" }, value: "≈ 10–13 : 1" },
      { label: { ko: "점화 순서", en: "Firing order", ja: "点火順序", zh: "点火顺序" }, value: "1-3-4-2" },
      { label: { ko: "최고 회전수", en: "Max speed", ja: "最高回転数", zh: "最高转速" }, value: "≈ 6,500 rpm" },
      { label: { ko: "효율", en: "Efficiency", ja: "効率", zh: "效率" }, value: "≈ 30–40 % (thermal)" },
    ] },
