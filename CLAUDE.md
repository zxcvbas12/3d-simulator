...
## 현재 상태 (중요 — 새 세션은 여기부터)
**1~9단계 완료 — 라이브 배포 중.** 지금 동작하는 것: React 셸 · 공통 엔진 `<Viewer>` · **모델 7종(학습 가능)** — 반도체 HBM/GPU/CPU · 우주 로켓엔진/지구관측위성/통신위성/재진입캡슐 · 디자인 토큰+웹폰트 · 공통 페이지 콘텐츠(4개 언어) · 푸터·상태 디자인 · 메타/OG. **Vercel 배포**(`main` push 자동) — <https://3d-simulator-rouge.vercel.app>. Lighthouse 95/100/100/100(컨테이너 측정).
다음은 **카테고리 확장**(자동차·항공·의료 등, 아래 진행 순서 참고). 어떤 작업이든:
...
- **배포**: **Vercel(확정·라이브).** GitHub 연동 → `main` push 시 자동 빌드·배포(설정 `vercel.json`, SPA fallback). 토큰/`.vercel/`은 `.gitignore`로 차단. ⚠️ 배포 검증 시 짧은 간격 `curl` 폴링은 Vercel 봇 챌린지(403 `x-vercel-mitigated`)를 유발 — 브라우저로 확인하거나 폴링 간격을 길게.
...
- **구동 연출**: 모델 `update` 훅이 `dt`·`autoRotate`를 받는다 — 회전·왕복 같은 연출은 **자동 회전 중에만**(연속 렌더 보장, reduced-motion이면 자동회전 차단으로 접근성 자동 충족). 예: 모터 회전자, 엔진 크랭크·피스톤.
- **모델 옵션**: `ModelDef.options` 선언 → 뷰어 좌상단 토글 UI 자동 표시, 선택값은 `update`의 `options`로. 모델은 부품 `visible` 전환·재질 클리핑 등으로 반응(예: HBM 층수·단면, GPU 스택 수·리드). **숨긴 부품은 엔진이 픽·프레이밍에서 자동 제외**, 옵션은 초기화·재진입 시 기본값 복귀. 값 라벨은 언어 중립("8-Hi"), 옵션 이름은 다국어.
...
## 정보 패널 형식 (전 모델 공통 — 구현됨)
- **tag·spec** — 부품 종류 라벨 + 실제 치수(언어 중립)
- **lead** — 한 줄 쉬운 요약
- **detail** — 어떻게/왜 중요한지 (간단히↔자세히 토글)
- **facts[]** — 짧은 항목 3개
- **sources[]** — 더 읽기: 표준·백서·신뢰 가능한 문서 링크(신뢰 레이어)
DRAM 등 같은 종류 부품은 클릭 시 "몇 번째"인지 표시.

## 모델 품질 체크리스트 (전 카테고리 공통 — 새 모델은 전부 충족할 것)
반도체 3종(HBM/GPU/CPU)이 기준. 다른 분야 모델도 같은 수준으로:
- [ ] **형상** — PBR 멀티머티리얼 + 공용 절차 텍스처(캐시 모듈, 새 무늬는 파라미터화해 추가) + 엣지 라인. 반복 요소는 인스턴싱.
- [ ] **분해** — 부품별 explode 벡터 + order(stagger). 동적 연출(관통·2단계 등)은 `update` 훅.
- [ ] **픽** — 학습 대상 부품 전부에 partId(인스턴스 메시 포함), 장식 요소는 픽 제외(`raycast=()=>{}` 또는 부모 귀속).
- [ ] **설명(data.ts)** — 부품마다 tag·title·**spec(실제 치수)**·lead·detail·facts(3개)·**sources(최소 1개)**, 4개 언어.
- [ ] **카탈로그** — overview 문단 + specs 표(5~7행) + `status: "live"`. 분야 첫 모델이면 `intro`(+`guide`)도.
- [ ] **문서** — 모델 CLAUDE.md(구현 사양·남은 보강) + README.md(사람용).
- [ ] **검증** — 빌드 + 브라우저(분해·부품 클릭·패널·모델 재진입·콘솔 에러 0) 후 커밋.
...
- **구현됨**: 메타/OG/파비콘/robots.txt + **og:url·canonical**(Vercel 프로덕션 도메인, og:image 절대경로, twitter:card). 커스텀 도메인 붙이면 갱신.
...
## 진행 순서
**1~9단계 완료** — ① 현황 점검 ② React+R3F 전환 ③ 토큰+앱 셸 ④ 공통 엔진 `<Viewer>` ⑤ HBM ⑥ GPU/CPU(평면·혼합 분해) ⑦ 성능+접근성 패스 ⑧ 사이트 폴리시(Lighthouse 95/100/100/100) ⑨ **배포**(Vercel 라이브 + `main` push 자동 배포 + og:url/canonical).

**카테고리 진행** — 반도체(HBM/GPU/CPU) · **우주 완성**(로켓 엔진 → 지구관측/통신 위성[공유 코어 `space/satellite/`] → 재진입 캡슐: 발사→궤도→귀환). 나머지 6개 분야는 catalog `soon`.

**다음 — 카테고리 확장.** 우선순위는 제작자가 정한다. 착수 전 무엇을 할지 확인받을 것(계획→승인→구현). 후보:
   a) **새 카테고리/모델** — 자동차·항공·가전·의료·에너지·로보틱스. 추가 절차는 "폴더 구조"의 3곳 규칙. 새 분야 첫 모델이면 카탈로그 `intro/guide` + 카테고리 CLAUDE.md부터. 모델 품질 기준은 HBM. **비슷한 변형이 여럿이면 위성처럼 공유 코어로**(중복 위로).
   b) **기존 모델 보강** — 각 모델 CLAUDE.md "남은 보강" 참고(HBM 단면/층수, GPU 스택 수 등).
   c) **(필요해지면)** Astro 이전 + SEO/애널리틱스 — 검색 유입이 중요해질 때.
