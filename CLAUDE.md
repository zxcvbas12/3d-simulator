...
## 현재 상태 (중요 — 새 세션은 여기부터)
**1~9단계 완료 — 라이브 배포 중.** 지금 동작하는 것: React 셸 · 공통 엔진 `<Viewer>`(분해·픽·**모델 옵션**·**구동 연출**·**단면**) · **모델 10종(학습 가능)** — 반도체 HBM/GPU/CPU · 우주 로켓엔진/지구관측위성/통신위성/재진입캡슐 · 자동차 EV배터리/구동모터/내연기관 · 디자인 토큰+웹폰트 · 공통 페이지 콘텐츠(4개 언어) · 푸터·상태 디자인 · 메타/OG. **Vercel 배포**(`main` push 자동) — <https://3d-simulator-rouge.vercel.app>. Lighthouse 95/100/100/100(컨테이너 측정).
다음은 **카테고리 확장**(항공·가전·의료·에너지·로보틱스, 아래 진행 순서 참고). 어떤 작업이든:
...
**카테고리 진행** — **반도체 완성**(HBM/GPU/CPU + 층수·단면·스택 수 옵션) · **우주 완성**(로켓 엔진 → 지구관측/통신 위성[공유 코어 `space/satellite/`] → 재진입 캡슐: 발사→궤도→귀환) · **자동차 완성**(EV 배터리 → 구동 모터 → 내연기관: 저장→구동→대비, 구동 연출·단면·셀 형식 옵션). 나머지 5개 분야(가전·항공·의료·에너지·로보틱스)는 catalog `soon`.

**다음 — 카테고리 확장.** 우선순위는 제작자가 정한다. 착수 전 무엇을 할지 확인받을 것(계획→승인→구현). 후보:
   a) **새 카테고리/모델** — 항공·가전·의료·에너지·로보틱스. 추가 절차는 "폴더 구조"의 3곳 규칙. 새 분야 첫 모델이면 카탈로그 `intro/guide` + 카테고리 CLAUDE.md부터. 모델 품질 기준은 HBM. **비슷한 변형이 여럿이면 위성처럼 공유 코어로**(중복 위로). (추천 첫 후보: 에너지·리튬이온 셀 — EV 배터리의 셀 내부로 이어짐)
   b) **기존 모델 보강** — 각 모델 CLAUDE.md "남은 보강" 참고. 큰 항목(sources·옵션·구동 연출·단면)은 완료 — 남은 것은 소형(엔진 밸브 분리, 배터리 모듈 컷어웨이, 모터 인버터 등).
