...
## 현재 상태 (중요)
**이미 Vite + TypeScript + 바닐라 Three.js로 시작된 프로젝트**다. 지금부터 **R3F로 전환**한다.
- 갈아엎지 않는다. 기존의 형상 정의·분해 계산·재질 설정 같은 알맹이는 **재활용**하고, Three.js를 직접 호출하던 부분만 R3F가 대신하게 바꾼다.
- 전환은 한 번에 하지 말고, 더미/HBM 하나를 R3F로 띄워 검증한 뒤 확장한다.
